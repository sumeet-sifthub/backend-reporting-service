...
class AnswerUsageLogsProcessor(UsageLogsTypeProcessor):
    # Processor for Answer usage logs with streaming batch processing
    
    async def process_export(self, message: SQSExportRequest) -> Optional[Union[BytesIO, Dict[str, str]]]:
        try:
            logger.info(f"Processing Answer usage logs export with streaming batches for event: {message.eventId}")
            ...
            if result:
                logger.info(f"Successfully generated streaming Answer usage logs export for event: {message.eventId}")
                logger.info(f"S3 Key: {result['s3_key']}, Download URL ready for Firebase notification")
                return result
            else:
                logger.error(f"Failed to generate streaming Answer usage logs export for event: {message.eventId}")
                return None
                
        except Exception as e:
            logger.error(f"Error processing streaming Answer usage logs export: {e}", exc_info=True)
            return None 