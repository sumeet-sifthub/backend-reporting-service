...
async def load_user_role_data_by_user_id(user_id: int, client_id: int, product_id: int):
    try:
        ...
        logger.error(f"Error loading user role data: {e}")
        # Return placeholder data if service call fails
        return {
            "userGuid": f"user_{user_id}",
            "clientGuid": f"client_{client_id}",
            "productGuid": f"product_{product_id}",
            "user_id": user_id,
            "client_id": client_id,
